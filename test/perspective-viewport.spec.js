...
import {PerspectiveViewport} from 'viewport-mercator-project';
...
test('PerspectiveViewport#constructor', t => {
  t.ok(new PerspectiveViewport() instanceof PerspectiveViewport,
    'Created new PerspectiveViewport with default args');
  t.end();
});

test('PerspectiveViewport#constructor - 0 width/height', t => {
  const viewport = new PerspectiveViewport({
    ...TEST_VIEWPORTS.mapState,
    width: 0,
    height: 0
  });
  t.ok(viewport instanceof PerspectiveViewport,
    'PerspectiveViewport constructed successfully with 0 width and height');
  t.end();
});

test('PerspectiveViewport.projectFlat', t => {
  for (const vc of TEST_VIEWPORTS) {
    const viewport = new PerspectiveViewport(vc.mapState);
...
test('PerspectiveViewport.project#3D', t => {
  for (const vc of TEST_VIEWPORTS) {
    const viewport = new PerspectiveViewport(vc.mapState);
    for (const tc of TEST_VIEWPORTS) {
      const {mapState} = tc;
...
// test('PerspectiveViewport.project#2D', t => {
...
test('PerspectiveViewport.getScales', t => {
  for (const vc of TEST_VIEWPORTS) {
    const viewport = new PerspectiveViewport(vc.mapState);
...
test('PerspectiveViewport.meterDeltas', t => {
  for (const vc of TEST_VIEWPORTS) {
    const viewport = new PerspectiveViewport(vc.mapState);
    for (const tc of TEST_VIEWPORTS) {
      const {mapState} = tc;
